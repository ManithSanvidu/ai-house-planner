...
def test_preferred_plan_restricts_existing_ai_candidate_pool(monkeypatch):
    ...
    monkeypatch.setattr(tool, 'load_base_plan_catalog', lambda: [])
...
def test_unknown_preferred_plan_is_rejected(monkeypatch):
    ...
    try:
        ...
        assert 'selected base plan is not compatible' in str(exc).lower()
    else:
        raise AssertionError('Expected incompatible selected plan to be rejected')
