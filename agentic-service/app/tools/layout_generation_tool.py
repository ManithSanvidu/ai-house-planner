...
def _mock_layout(
    ...
    if template and template.get("layout"):
        for key, values in template["layout"].items():
            ...
            # Assign upper bedrooms to floor 2 if multi-floor
            assigned_floor = 1
            if floors > 1 and room_type in ("bedroom_2", "bedroom_3", "bedroom_4", "bedroom_5"):
                assigned_floor = 2
        ...
        for i in range(1, bedrooms + 1):
            assigned_floor = 2 if (floors > 1 and i > 1) else 1
            rooms.append(RoomLayout(
    ...
    current_bedrooms = len([r for r in rooms if r.room_type.startswith("bedroom")])
    if current_bedrooms < bedrooms:
        for i in range(current_bedrooms + 1, bedrooms + 1):
            assigned_floor = 2 if floors > 1 else 1
            rooms.append(RoomLayout(
                room_type=f"bedroom_{i}",
                name=f"Bedroom {i}",
                floor=assigned_floor,
