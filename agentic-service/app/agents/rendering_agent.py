...
def rendering_node(state: WorkflowState) -> WorkflowState:
    ...
    try:
        ...
        preferences = state.input_data.preferences if state.input_data else {}
        style = preferences.get('architecturalStyle', 'Modern Design') if isinstance(preferences, dict) else 'Modern Design'
        ...
        print("[Rendering Agent] matplotlib not installed, skipping image export.")
        state.execution_log.append({
            "agent_name": "RenderingAgent",
            "action": "Render plan skipped (matplotlib not installed)",
            "result": "skipped",
            "created_at_utc": "now"
        })
