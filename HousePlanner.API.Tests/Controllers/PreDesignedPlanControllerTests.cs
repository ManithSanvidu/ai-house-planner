...
public class PreDesignedPlanControllerTests
{
    ...
    private static Mock<ICurrentUserContextService> User(string? role="User") { var mock=new Mock<ICurrentUserContextService>(); mock.Setup(x=>x.GetAsync(It.IsAny<HttpContext>())).ReturnsAsync(role is null?null:new CurrentUserContext("test@example.com",role)); return mock; }
