   ...
   `}>
    <div>
     <div className="flex items-center justify-between px-3 mb-1">
      <h2 className="text-[10px] font-bold text-text-secondary text-text-muted uppercase tracking-wider">Navigation</h2>
      {onClose && (
       <button onClick={onClose} className="md:hidden p-1 text-text-muted hover:text-zinc-800 dark:hover:text-zinc-200">
        <X size={16} />
       </button>
      )}
     </div>
      ...
      {links.map((link) => {
       ...
       return (
        ...
        >
         {({ isActive }) => (
          <>
           <Icon size={18} className={isActive ? 'text-indigo-600 dark:text-indigo-400' : 'text-text-secondary text-text-muted'} />
    ...
    <div className="mt-auto border-t border-zinc-100 dark:border-border-strong/50 pt-4 px-3 text-center bg-surface-elevated/50 bg-surface/30 rounded-xl pb-2 transition-colors duration-300">
     <p className="text-[11px] text-text-secondary text-text-muted font-medium leading-normal">
      Logged in as <span className="font-bold text-zinc-700 dark:text-gray-300 bg-surface-elevated px-2 py-0.5 rounded shadow-sm border border-zinc-100 border-border ml-1">{user.role}</span>
     </p>
    </div>
   </aside>
  </>
