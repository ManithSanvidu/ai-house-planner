...
const DesignCard = ({ design, workflow, compared, compareFull, onCompare, onSelect, onUnselect, onRemove }: { design: DesignHistoryDto; workflow: WorkflowDesignHistoryDto; compared: boolean; compareFull: boolean; onCompare: () => void; onSelect: () => void; onUnselect: () => void; onRemove: () => void }) => {
  ...
  return <article className={`rounded-2xl bg-white dark:bg-gray-950 border p-4 ${design.isPreferred ? 'border-emerald-500 ring-1 ring-emerald-500' : compared ? 'border-indigo-500 ring-1 ring-indigo-500' : 'border-zinc-200 dark:border-gray-700'}`}>
    <div className="flex justify-between gap-2"><div><span className="text-xs uppercase text-zinc-400 font-bold">Version {design.version}</span><h3 className="font-bold mt-1">{design.topology?.replaceAll('_', ' ') || 'House design'}</h3></div>{design.isPreferred && <span className="text-emerald-600 flex gap-1 text-xs font-bold"><CheckCircle2 size={16}/>Selected</span>}</div>
    <MiniPlan design={design}/>
    <dl className="grid grid-cols-2 gap-2 text-sm my-3"><div><dt className="text-zinc-400">Rooms</dt><dd>{design.bedrooms} bed · {design.bathrooms} bath</dd></div><div><dt className="text-zinc-400">Floors</dt><dd>{design.floorCount}</dd></div><div><dt className="text-zinc-400">Area</dt><dd>{design.totalBuiltUpAreaSqft} sq ft</dd></div><div><dt className="text-zinc-400">Mode</dt><dd className="truncate">{design.generationMode?.replaceAll('_',' ') || 'Generated'}</dd></div></dl>
