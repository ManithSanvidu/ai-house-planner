...
const IntakeForm: React.FC = () => {
  ...
  if (isSuccess) {
    return (
      <div className="min-h-[calc(100vh-65px)] flex items-center justify-center p-6 bg-gray-50 dark:bg-gray-950 transition-colors">
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-white dark:bg-gray-900 rounded-[2rem] p-10 max-w-md w-full text-center shadow-xl border border-gray-100 dark:border-gray-800">
          <div className="w-20 h-20 bg-green-100 dark:bg-green-900/30 rounded-full flex items-center justify-center mx-auto mb-6">
            <CheckCircle2 size={40} className="text-green-600 dark:text-green-400" />
          </div>
          <h2 className="text-3xl font-bold text-gray-900 dark:text-white mb-4">Project Created!</h2>
  ...
  const renderStepIndicator = () => (
    <div className="flex items-center justify-between mb-8 sm:mb-12 max-w-2xl mx-auto px-2 relative">
      <div className="absolute top-1/2 left-0 right-0 h-[2px] bg-gray-200 dark:bg-gray-800 -z-10 -translate-y-1/2"></div>
      {steps.map((step) => (
        <div key={step.id} className="flex flex-col items-center gap-2">
          <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold transition-colors ${currentStep >= step.id ? 'bg-blue-600 text-white' : 'bg-gray-100 dark:bg-gray-800 text-gray-400 border border-gray-200 dark:border-gray-700'}`}>
            {currentStep > step.id ? <Check size={14} /> : step.id}
          </div>
          <span className={`text-[10px] uppercase font-bold tracking-wider hidden sm:block ${currentStep >= step.id ? 'text-gray-900 dark:text-white' : 'text-gray-400 dark:text-gray-600'}`}>
            {step.title}
          </span>
        </div>
      ))}
  ...
  return (
    <div className="min-h-[calc(100vh-65px)] bg-[#fcfcfd] dark:bg-gray-950 p-4 sm:p-8 flex items-start justify-center transition-colors">
      <div className="max-w-3xl w-full">
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-white dark:bg-gray-900 rounded-[2rem] shadow-sm border border-gray-200 dark:border-gray-800 overflow-hidden flex flex-col min-h-[600px]">
          ...
          <div className="p-6 sm:p-10 flex-1 relative">
            ...
            {currentStep === 1 && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8">
                <div>
                  <h2 className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white mb-2">Tell us about your land</h2>
                  <p className="text-gray-500 dark:text-gray-400">Basic details about your property size and terrain.</p>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  <div>
                    ...
                    <div className="flex gap-2">
                      <input 
                        ...
                        className={`flex-1 bg-gray-50 dark:bg-gray-800 border ${fieldErrors.landSize ? 'border-red-500' : 'border-gray-200 dark:border-gray-700'} rounded-xl px-4 py-3 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 outline-none`} 
                        placeholder="e.g. 25"
                      />
                      <select name="landUnit" value={formData.landUnit} onChange={handleInputChange} className="w-32 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl px-4 py-3 text-gray-900 dark:text-white outline-none">
                  ...
                  <div>
                    ...
                    <select name="terrainType" value={formData.terrainType} onChange={handleInputChange} className="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl px-4 py-3 text-gray-900 dark:text-white outline-none">
            ...
            {currentStep === 2 && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8">
                <div>
                  <h2 className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white mb-2">Define the plot</h2>
                  <p className="text-gray-500 dark:text-gray-400">If dimensions are unavailable, the system can estimate conceptual proportions from land area.</p>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  <div>
                    <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">Plot Width (ft)</label>
                    <input type="number" name="plotWidth" value={formData.plotWidth} onChange={handleInputChange} className="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl px-4 py-3 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 outline-none" placeholder="Optional" />
                  </div>
                  <div>
                    <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">Plot Length (ft)</label>
                    <input type="number" name="plotLength" value={formData.plotLength} onChange={handleInputChange} className="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl px-4 py-3 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 outline-none" placeholder="Optional" />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div>
                    ...
                    <select name="roadSide" value={formData.roadSide} onChange={handleInputChange} className="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl px-4 py-3 text-gray-900 dark:text-white outline-none">
                  ...
                  <div>
                    ...
                    <select name="northDirection" value={formData.northDirection} onChange={handleInputChange} className="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl px-4 py-3 text-gray-900 dark:text-white outline-none">
                  ...
                  <div>
                    ...
                    <select name="entranceSide" value={formData.entranceSide} onChange={handleInputChange} className="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl px-4 py-3 text-gray-900 dark:text-white outline-none">
                ...
                <div className="border border-gray-200 dark:border-gray-800 rounded-2xl overflow-hidden">
                  <button type="button" onClick={() => setShowAdvancedPlot(!showAdvancedPlot)} className="w-full bg-gray-50 dark:bg-gray-800/50 p-4 flex items-center justify-between text-sm font-bold text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors">
                    Advanced Plot Constraints {showAdvancedPlot ? <Expand className="rotate-180" size={16} /> : <Expand size={16} />}
                  </button>
                  <AnimatePresence>
                    {showAdvancedPlot && (
                      <motion.div initial={{ height: 0 }} animate={{ height: 'auto' }} exit={{ height: 0 }} className="overflow-hidden">
                        <div className="p-4 grid grid-cols-2 sm:grid-cols-4 gap-4 bg-white dark:bg-gray-900">
                          {['front', 'rear', 'left', 'right'].map((side) => (
                            <div key={side}>
                              <label className="block text-xs font-bold text-gray-500 dark:text-gray-400 mb-1 capitalize">{side} Setback (ft)</label>
                              <input type="number" name={`${side}Setback`} value={(formData as any)[`${side}Setback`]} onChange={handleInputChange} className="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg px-3 py-2 text-gray-900 dark:text-white outline-none" />
                            </div>
                          ))}
                        </div>
                      </motion.div>
            ...
            {currentStep === 3 && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8">
                <div>
                  <h2 className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white mb-2">Describe the home you want</h2>
                  <p className="text-gray-500 dark:text-gray-400">Basic requirements for the internal spaces.</p>
                </div>

                {compatibility?.supported && (
                  <div className="text-sm text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-gray-800/50 px-4 py-3 rounded-xl border border-gray-100 dark:border-gray-800 flex gap-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        ...
                        return (
                          <button
                            ...
                            className={`w-12 h-12 rounded-xl border flex items-center justify-center font-bold text-sm transition-all
                              ${!isSupported ? 'opacity-40 border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 cursor-not-allowed text-gray-400' 
                  ...
                  <div className="sm:col-span-2">
                    ...
                    <select name="architecturalStyle" value={formData.architecturalStyle} onChange={handleInputChange} className="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl px-4 py-3 text-gray-900 dark:text-white outline-none">
            ...
            {currentStep === 4 && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8">
                <div>
                  <h2 className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white mb-2">Choose optional features</h2>
                  <p className="text-gray-500 dark:text-gray-400">Select any extra requirements for your home.</p>
                </div>
                    ...
                    return (
                      <div 
                        ...
                        className={`cursor-pointer border rounded-xl p-4 flex gap-4 transition-all
                          ${!isSupported ? 'opacity-40 border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50 cursor-not-allowed' :
                            formData[feature.key as keyof IntakeFormData] ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20' : 'border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800'}
                        `}
                        title={reason || (!isSupported ? 'Not available' : '')}
                      >
                        <div className={`mt-1 flex-shrink-0 ${formData[feature.key as keyof IntakeFormData] ? 'text-blue-600 dark:text-blue-400' : 'text-gray-400'}`}>
                          {formData[feature.key as keyof IntakeFormData] ? <CheckSquare size={20} /> : <div className={`w-5 h-5 border-2 rounded ${!isSupported ? 'border-gray-200 dark:border-gray-700' : 'border-gray-300 dark:border-gray-600'}`} />}
                        </div>
                        <div>
                          <h4 className={`font-bold text-sm ${formData[feature.key as keyof IntakeFormData] ? 'text-blue-900 dark:text-blue-200' : 'text-gray-900 dark:text-white'}`}>{feature.label}</h4>
                          <p className={`text-xs mt-1 ${formData[feature.key as keyof IntakeFormData] ? 'text-blue-700 dark:text-blue-300' : 'text-gray-500 dark:text-gray-400'}`}>
            ...
            {currentStep === 5 && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8">
                <div>
                  <h2 className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white mb-2">Review your project</h2>
                  <p className="text-gray-500 dark:text-gray-400">Ensure all details are correct before generating your plan.</p>
                </div>
                ...
                <div className="space-y-6">
                  <div className="bg-gray-50 dark:bg-gray-800/50 rounded-2xl p-6 border border-gray-100 dark:border-gray-800">
                    <div className="flex justify-between items-center mb-4">
                      <h3 className="font-bold text-gray-900 dark:text-white flex items-center gap-2"><Map size={18} className="text-gray-400" /> Land</h3>
                  ...
                  <div className="bg-gray-50 dark:bg-gray-800/50 rounded-2xl p-6 border border-gray-100 dark:border-gray-800">
                    <div className="flex justify-between items-center mb-4">
                      <h3 className="font-bold text-gray-900 dark:text-white flex items-center gap-2"><Compass size={18} className="text-gray-400" /> Plot & Site</h3>
                  ...
                  <div className="bg-gray-50 dark:bg-gray-800/50 rounded-2xl p-6 border border-gray-100 dark:border-gray-800">
                    <div className="flex justify-between items-center mb-4">
                      <h3 className="font-bold text-gray-900 dark:text-white flex items-center gap-2"><Home size={18} className="text-gray-400" /> House Requirements</h3>
                  ...
                  <div className="bg-gray-50 dark:bg-gray-800/50 rounded-2xl p-6 border border-gray-100 dark:border-gray-800">
                    <div className="flex justify-between items-center mb-4">
                      <h3 className="font-bold text-gray-900 dark:text-white flex items-center gap-2"><CheckCircle2 size={18} className="text-gray-400" /> Features</h3>
                      <button onClick={() => setCurrentStep(4)} className="text-xs font-bold text-blue-600 dark:text-blue-400 hover:underline">Edit</button>
                    </div>
                    <ul className="text-sm text-gray-600 dark:text-gray-300 flex flex-wrap gap-2">
          ...
          <div className="p-6 sm:p-10 bg-gray-50 dark:bg-gray-800/30 border-t border-gray-100 dark:border-gray-800 flex items-center justify-between">
            <button 
              type="button" 
              onClick={handleBack} 
              disabled={currentStep === 1 || isSubmitting}
              className={`flex items-center gap-2 px-6 py-3 rounded-xl font-bold text-sm transition-colors ${currentStep === 1 || isSubmitting ? 'opacity-0 pointer-events-none' : 'text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700 border border-gray-200 dark:border-gray-700'}`}
            >
            ...
            {currentStep < 5 ? (
              ...
              <button 
                ...
                className="flex items-center justify-center gap-3 px-8 py-4 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold text-sm transition-colors shadow-sm disabled:opacity-70 min-w-[200px]"
              >
                {isSubmitting ? (
                  <>
