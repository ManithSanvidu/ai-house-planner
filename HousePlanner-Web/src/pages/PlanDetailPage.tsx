  ...
  <main className="p-4 md:p-8 lg:p-10 max-w-7xl mx-auto text-zinc-900 dark:text-text-primary space-y-8">
   ...
   <div className="grid lg:grid-cols-[minmax(0,1fr)_340px] gap-8">
     ...
     <section className="h-[500px] md:h-[700px] min-w-0 rounded-2xl overflow-hidden border border-border dark:border-border-strong relative bg-surface bg-background shadow-sm">
      ...
      {plan.floorCount === 1 && (
       <div className="absolute z-10 m-4">
        <span className="px-4 py-2 rounded-xl text-sm font-bold shadow-sm bg-surface text-zinc-700 dark:text-zinc-300 border border-border">Ground Floor</span>
       </div>
      )}
    ...
    <aside className="space-y-6">
     ...
     <section className="bg-surface border border-zinc-100 dark:border-border-strong rounded-2xl p-6 shadow-sm">
      ...
      <div className="space-y-3 text-sm">
       {[
        ...
        <div key={k} className="flex justify-between gap-3 border-b border-zinc-100 dark:border-border-strong pb-2 last:border-0 last:pb-0">
         <span className="text-text-muted dark:text-text-secondary">{k}</span>
         <b className="text-right text-zinc-900 dark:text-zinc-100">{v}</b>
        </div>
       ))}
      </div>

      <div className="mt-5 pt-5 border-t border-zinc-100 dark:border-border-strong">
       <h3 className="text-xs font-bold text-text-secondary uppercase tracking-wider mb-3">Supported Features</h3>
       <ul className="flex flex-wrap gap-2">
        {[
         ...
         <li key={f.key} className="bg-surface-muted bg-surface-elevated text-zinc-700 dark:text-zinc-300 px-2.5 py-1 rounded text-xs font-medium border border-border">{f.label}</li>
        ))}
        {['hasSeparateDining', 'hasOffice', 'isAccessibleFriendly'].filter(k => (plan as any)[k]).length === 0 && (
         <li className="text-xs text-text-muted">Standard layout only</li>
        )}
       </ul>
      </div>
     </section>
      ...
      {(constructionState === 'NO_REQUEST' || constructionState === 'DECLINED' || constructionState === 'CANCELLED') && !showConstructorSelect && (
       <div>
        ...
        <button
         ...
         className="w-full bg-indigo-600 hover:bg-indigo-700 text-text-primary font-bold py-3 px-4 rounded-xl transition-colors shadow-sm"
        >
         Request Constructor
        </button>
       </div>
       ...
       <div className="space-y-4">
        ...
        {selectedConstructor && (
         <button
          ...
          className="w-full bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-text-primary font-bold py-3 px-4 rounded-xl transition-colors shadow-sm"
         >
          {submittingRequest ? 'Sending...' : `Send construction request to ${constructors.find(c => c.id === selectedConstructor)?.name}?`}
         </button>
        )}
        <button onClick={() => setShowConstructorSelect(false)} className="w-full py-2 text-sm text-text-muted hover:text-zinc-800 dark:hover:text-zinc-300">
         Cancel
        </button>
       </div>
      )}
      ...
      {constructionState === 'ACCEPTED_OR_ACTIVE' && (
       <div>
        <button
         ...
         className="w-full bg-emerald-600 hover:bg-emerald-700 text-text-primary font-bold py-3 px-4 rounded-xl transition-colors shadow-sm"
        >
     ...
     <p className="text-xs text-text-muted bg-surface-elevated bg-background p-4 rounded-xl border border-zinc-100 dark:border-border-strong">
