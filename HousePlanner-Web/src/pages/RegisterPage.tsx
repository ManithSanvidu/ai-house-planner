...
const RegisterPage: React.FC = () => {
 ...
 const fieldClass = 'block w-full pl-11 pr-4 py-3.5 bg-gray-50 bg-background border border-border dark:border-border-strong rounded-xl text-gray-900 dark:text-text-primary focus:ring-2 focus:ring-blue-600 focus:border-transparent outline-none';
 return <div className="min-h-screen bg-background flex items-center justify-center p-6 relative overflow-hidden">
  ...
  <main className="w-full max-w-md relative z-10 bg-surface rounded-3xl p-8 sm:p-10 custom-shadow-xl border border-gray-100 dark:border-border-strong">
   <h1 className="text-2xl font-bold text-gray-900 dark:text-text-primary">Create your HousePlanner account</h1>
   <p className="text-sm text-text-muted text-text-secondary mt-2 mb-7">Create a customer account to explore plans and manage your home design.</p>
   ...
   <form onSubmit={submit} className="space-y-5">
    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Full Name<div className="relative mt-1.5"><User className="absolute left-4 top-3.5 text-text-secondary" size={20}/><input id="reg-fullname" required autoComplete="name" value={fullName} onChange={e=>setFullName(e.target.value)} className={fieldClass} placeholder="Kasun Perera"/></div></label>
    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Email Address<div className="relative mt-1.5"><Mail className="absolute left-4 top-3.5 text-text-secondary" size={20}/><input id="reg-email" required type="email" autoComplete="email" value={email} onChange={e=>setEmail(e.target.value)} className={fieldClass} placeholder="name@example.com"/></div></label>
    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Password<div className="relative mt-1.5"><Lock className="absolute left-4 top-3.5 text-text-secondary" size={20}/><input id="reg-password" required minLength={6} type="password" autoComplete="new-password" value={password} onChange={e=>setPassword(e.target.value)} className={fieldClass} placeholder="At least 6 characters"/></div></label>
    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Confirm Password<div className="relative mt-1.5"><Lock className="absolute left-4 top-3.5 text-text-secondary" size={20}/><input id="reg-confirm-password" required type="password" autoComplete="new-password" value={confirmPassword} onChange={e=>setConfirmPassword(e.target.value)} className={fieldClass} placeholder="••••••••"/></div></label>
    <button id="register-submit" className="w-full bg-gray-900 dark:bg-surface text-text-primary dark:text-gray-900 font-bold py-4 rounded-xl flex items-center justify-center gap-2 focus-visible:ring-4 focus-visible:ring-blue-300">Create Account <ArrowRight size={18}/></button>
   </form>
   ...
   <button type="button" onClick={continueWithGoogle} className="w-full border border-border-strong border-border py-3 rounded-xl font-semibold focus-visible:ring-4 focus-visible:ring-blue-200">Continue with Google</button>
