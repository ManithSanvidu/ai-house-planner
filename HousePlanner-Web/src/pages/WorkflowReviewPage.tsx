...
export const WorkflowReviewPage: React.FC = () => {
 ...
 return (
  <div className="min-h-[calc(100vh-65px)] bg-slate-50 text-zinc-900">
   <header className="bg-surface border-b border-border px-5 py-6 md:px-8">
    <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-center md:justify-between gap-4">
     <div>
      ...
      <p className="text-sm text-text-muted mt-2">Designed for {site === 'Not specified' ? 'your site' : `${site.toLowerCase()} land`} with a practical {topology.toLowerCase()} layout.</p>
     </div>
   ...
   {/* ─── Sidebar: Details ─── */}
   {isSidebarOpen && (
    <aside className="w-full xl:w-80 bg-surface border-b xl:border-b-0 xl:border-r border-border flex flex-col shrink-0 z-10">
     <div className="p-6 border-b border-zinc-100 flex justify-between items-center bg-gradient-to-b from-zinc-50/50 to-transparent">
      <div>
       ...
       <p className="text-sm text-text-muted mt-1">The key details of this design.</p>
      </div>
     </div>
    ...
    <div className="p-6 space-y-5 flex-1">
     ...
     <div className="bg-surface-elevated p-4 rounded-xl border border-zinc-100 shadow-sm">
      <dl className="text-sm text-zinc-700 space-y-3">
       <div className="flex justify-between gap-3"><dt className="font-medium text-text-muted">Bedrooms</dt><dd className="font-bold">{bedroomCount}</dd></div>
       <div className="flex justify-between gap-3"><dt className="font-medium text-text-muted">Bathrooms</dt><dd className="font-bold">{bathroomCount}</dd></div>
       <div className="flex justify-between gap-3"><dt className="font-medium text-text-muted">Floors</dt><dd className="font-bold">{workflow.design.floorCount}</dd></div>
       <div className="flex justify-between gap-3"><dt className="font-medium text-text-muted">Area</dt><dd className="font-bold">{formatArea(workflow.design.totalBuiltUpAreaSqft)}</dd></div>
       <div className="border-t border-border pt-3 flex justify-between gap-3"><dt className="font-medium text-text-muted">Site</dt><dd className="font-bold text-right">{site}</dd></div>
       <div className="flex justify-between gap-3"><dt className="font-medium text-text-muted">Foundation</dt><dd className="font-bold text-right">{formatFoundation(workflow.design.foundationType)}</dd></div>
       <div className="flex justify-between gap-3"><dt className="font-medium text-text-muted">Layout</dt><dd className="font-bold text-right">{topology}</dd></div>
      </dl>
      {workflow.design.designScore != null && <details className="mt-4 border-t border-border pt-3 text-sm"><summary className="cursor-pointer font-semibold text-text-secondary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-400">Design details</summary><p className="mt-2 text-text-muted">Architectural quality score: {workflow.design.designScore}</p></details>}
     </div>
   ...
   {/* ─── Main Area ─── */}
   <div className="flex-1 min-w-0 flex flex-col relative bg-surface min-h-[560px]">
    {/* Top Action Bar */}
    <div className="bg-surface border-b border-slate-200 px-3 md:px-4 py-3 flex flex-wrap items-center gap-3">
     <button aria-label={isSidebarOpen ? 'Hide design summary' : 'Show design summary'} onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="p-2 text-text-muted hover:bg-slate-100 rounded-md transition-colors focus-visible:ring-2 focus-visible:ring-indigo-400" title="Toggle design summary">
      <Menu size={20} />
     </button>
     ...
     <div className="flex gap-1 bg-slate-100 p-1 rounded-lg">
      ...
      <button
       ...
       className={`px-4 py-1.5 rounded-md text-sm font-bold transition-all flex items-center gap-2 ${
        activeTab === 'construction' ? 'bg-surface text-indigo-600 shadow-sm' : 'text-text-muted hover:text-slate-700'
       }`}
      >
       🚧 Construction Plan
      </button>
       ...
       {floorNumbers.map(floor => (
        <button
         ...
         className={`px-4 py-1.5 rounded-lg text-sm font-medium transition-colors focus-visible:ring-4 focus-visible:ring-indigo-300 ${
          selectedFloor === floor
           ? 'bg-indigo-600 text-text-primary shadow-sm'
      ...
      <div className="p-8 max-w-4xl mx-auto">
       {!workflow.constructionPlan ? (
        ...
        <div className="space-y-8 animate-fade-in">
         ...
         {/* Summary Header */}
         <div className="bg-surface p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
           <div className="flex items-center gap-3 mb-1">
            <h2 className="text-2xl font-bold text-slate-800">Project Timeline Estimate</h2>
            {isAdmin && !isEditingPlan && (
             <button onClick={startEditingPlan} className="p-1.5 text-text-secondary hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors" title="Edit Plan">
              <Edit2 size={16} />
             </button>
            )}
           </div>
           <p className="text-text-muted">AI-generated construction roadmap based on architectural design</p>
          </div>
          <div className="text-right">
           <div className="text-3xl font-black text-indigo-600">
            {workflow.constructionPlan.project_summary.estimated_duration_days} <span className="text-lg text-text-secondary font-medium">days</span>
           </div>
         ...
         <div className="grid grid-cols-2 gap-4">
          <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
           <h4 className="text-xs uppercase tracking-widest text-text-secondary font-bold mb-1">Target Duration</h4>
           {isEditingPlan ? (
            <input
             type="number"
             value={editTargetDuration}
             onChange={e => setEditTargetDuration(e.target.value ? Number(e.target.value) : '')}
             className="w-full bg-surface border border-slate-300 rounded-lg px-3 py-2 text-sm font-semibold text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500"
             placeholder="e.g. 150"
            />
           ) : (
          ...
          <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
           <h4 className="text-xs uppercase tracking-widest text-text-secondary font-bold mb-1">Schedule Status</h4>
           {isEditingPlan ? (
            <select
             value={editScheduleStatus}
             onChange={e => setEditScheduleStatus(e.target.value)}
             className="w-full bg-surface border border-slate-300 rounded-lg px-3 py-2 text-sm font-bold focus:outline-none focus:ring-2 focus:ring-indigo-500"
            >
         ...
         {isEditingPlan && (
          <div className="flex justify-end gap-3 mt-4">
           ...
           <button onClick={handleSavePlan} disabled={isSavingPlan} className="px-4 py-2 rounded-lg bg-indigo-600 text-text-primary font-semibold disabled:opacity-50 flex items-center gap-2">
         ...
         <div>
          ...
          <div className="space-y-3">
           {workflow.constructionPlan.phases.map((phase: any) => (
            <div key={phase.id} className="bg-surface p-4 rounded-xl border border-slate-200 shadow-sm flex flex-col md:flex-row md:items-center justify-between hover:border-indigo-200 transition-colors gap-4">
             {editingPhaseId === phase.id ? (
              // Edit Phase Form
              <div className="w-full grid grid-cols-1 md:grid-cols-2 gap-4">
               <div className="space-y-3">
                <div><label className="text-xs font-bold text-text-muted uppercase">Phase Name</label><input type="text" value={phaseFormData.name} onChange={e => setPhaseFormData({...phaseFormData, name: e.target.value})} className="w-full border rounded px-3 py-2 text-sm mt-1" /></div>
                <div className="grid grid-cols-3 gap-2">
                 <div><label className="text-xs font-bold text-text-muted uppercase">Duration</label><input type="number" value={phaseFormData.duration_days} onChange={e => setPhaseFormData({...phaseFormData, duration_days: Number(e.target.value)})} className="w-full border rounded px-3 py-2 text-sm mt-1" /></div>
                 <div><label className="text-xs font-bold text-text-muted uppercase">Start Day</label><input type="number" value={phaseFormData.start_day} onChange={e => setPhaseFormData({...phaseFormData, start_day: Number(e.target.value)})} className="w-full border rounded px-3 py-2 text-sm mt-1" /></div>
                 <div><label className="text-xs font-bold text-text-muted uppercase">End Day</label><input type="number" value={phaseFormData.end_day} onChange={e => setPhaseFormData({...phaseFormData, end_day: Number(e.target.value)})} className="w-full border rounded px-3 py-2 text-sm mt-1" /></div>
                </div>
               </div>
               <div className="flex items-end justify-end gap-2 pb-1">
                <button onClick={cancelPhaseEdit} className="p-2 text-text-muted hover:bg-slate-100 rounded-lg"><X size={18}/></button>
                <button onClick={savePhase} disabled={isSavingPlan} className="px-4 py-2 bg-indigo-600 text-text-primary rounded-lg text-sm font-bold disabled:opacity-50 flex items-center gap-2"><Save size={16}/> Save</button>
               </div>
              </div>
             ) : (
              // View Phase
               ...
               <div className="flex items-center gap-6">
                ...
                {isAdmin && (
                 <div className="flex gap-2">
                  <button onClick={() => startEditingPhase(phase)} className="p-2 text-text-secondary hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors"><Edit2 size={16}/></button>
                  <button onClick={() => deletePhase(phase.id)} className="p-2 text-text-secondary hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors"><Trash2 size={16}/></button>
                 </div>
                )}
               </div>
              </>
           ...
           {/* Add Phase Form */}
           {isAddingPhase && (
            <div className="bg-surface p-4 rounded-xl border border-indigo-200 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div className="w-full grid grid-cols-1 md:grid-cols-2 gap-4">
               <div className="space-y-3">
                <div><label className="text-xs font-bold text-text-muted uppercase">Phase Name</label><input type="text" value={phaseFormData.name} onChange={e => setPhaseFormData({...phaseFormData, name: e.target.value})} className="w-full border rounded px-3 py-2 text-sm mt-1" placeholder="e.g. Foundation" /></div>
                <div className="grid grid-cols-3 gap-2">
                 <div><label className="text-xs font-bold text-text-muted uppercase">Duration</label><input type="number" value={phaseFormData.duration_days} onChange={e => setPhaseFormData({...phaseFormData, duration_days: Number(e.target.value)})} className="w-full border rounded px-3 py-2 text-sm mt-1" /></div>
                 <div><label className="text-xs font-bold text-text-muted uppercase">Start Day</label><input type="number" value={phaseFormData.start_day} onChange={e => setPhaseFormData({...phaseFormData, start_day: Number(e.target.value)})} className="w-full border rounded px-3 py-2 text-sm mt-1" /></div>
                 <div><label className="text-xs font-bold text-text-muted uppercase">End Day</label><input type="number" value={phaseFormData.end_day} onChange={e => setPhaseFormData({...phaseFormData, end_day: Number(e.target.value)})} className="w-full border rounded px-3 py-2 text-sm mt-1" /></div>
           ...
           {isAdmin && !isAddingPhase && (
            <button onClick={startAddingPhase} className="w-full py-4 border-2 border-dashed border-slate-300 rounded-xl text-text-muted font-bold hover:border-indigo-400 hover:text-indigo-600 hover:bg-indigo-50/50 transition-colors flex items-center justify-center gap-2 mt-4">
