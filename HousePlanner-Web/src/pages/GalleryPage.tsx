        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {plans.map((item, index) => {
            ...
            <motion.div
              ...
              key={item.id}
              className="relative group cursor-pointer rounded-2xl overflow-hidden aspect-square bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-800"
            ...
            >
              ...
              <div className="absolute inset-0 bg-black/0 group-hover:bg-black/60 transition-colors duration-300 flex flex-col items-center justify-center p-6 text-center">
                <span className="text-white opacity-0 group-hover:opacity-100 font-bold tracking-widest text-lg transition-opacity duration-300 mb-2">
        ...
        {selectedImg && (
          ...
          >
            <motion.div 
              ...
              className="bg-white dark:bg-gray-900 rounded-3xl overflow-hidden max-w-4xl w-full grid md:grid-cols-2 shadow-2xl relative"
              onClick={e => e.stopPropagation()}
            >
              <div className="h-[300px] md:h-[500px] p-8 bg-gray-50 dark:bg-gray-800 flex items-center justify-center">
              ...
              <div className="p-8 md:p-12 flex flex-col justify-center relative">
                <button onClick={() => setSelectedImg(null)} className="absolute top-6 right-6 text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors">
                  <X size={24} />
                </button>
                <div className="flex items-center gap-2 mb-4">
                  <Sparkles size={16} className="text-indigo-600 dark:text-indigo-400" />
                  <span className="text-[10px] font-bold tracking-[0.2em] text-gray-500 dark:text-gray-400">DESIGN PARAMETERS</span>
                </div>
                <h2 className="text-2xl md:text-3xl text-gray-900 dark:text-white font-serif leading-relaxed mb-6">
                  {getCustomerPlanName(selectedImg)}
                </h2>
                
                <div className="grid grid-cols-2 gap-y-4 gap-x-6 mb-8 text-sm text-gray-600 dark:text-gray-300">
                  <div className="flex flex-col">
                    <span className="text-[10px] text-gray-400 font-bold uppercase tracking-wider mb-1">Configuration</span>
                    <span>{selectedImg.bedrooms} Bed, {selectedImg.bathrooms} Bath</span>
                  </div>
                  <div className="flex flex-col">
                    <span className="text-[10px] text-gray-400 font-bold uppercase tracking-wider mb-1">Floors</span>
                  ...
                  <div className="flex flex-col">
                    <span className="text-[10px] text-gray-400 font-bold uppercase tracking-wider mb-1">Min. Land</span>
                    <span>{selectedImg.minimumLandSizePerches} Perches</span>
                  </div>
                  <div className="flex flex-col col-span-2">
                    <span className="text-[10px] text-gray-400 font-bold uppercase tracking-wider mb-1">Suitable Terrain</span>
                    <span className="capitalize">{selectedImg.suitableTerrain}</span>
                  </div>
                </div>

                <Link to={`/dashboard/plans/${selectedImg.id}`} className="bg-indigo-600 text-white px-6 py-4 rounded-xl text-xs font-bold tracking-[0.1em] hover:bg-indigo-700 transition-colors text-center w-full shadow-md">
                  VIEW FULL PLAN
                </Link>
              </div>
            </motion.div>
