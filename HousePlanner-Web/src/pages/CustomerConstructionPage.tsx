...
export default function CustomerConstructionPage() {
  ...
  const load = useCallback(async () => {
    const [overview, approved, available] = await Promise.all([customerConstructionService.overview(), customerConstructionService.approvedDesigns(), customerConstructionService.constructors()]);
    setData(overview); setDesigns(approved); setConstructors(available);
    if (!selectedDesign && approved.length) setSelectedDesign(approved[0].designId);
  }, [selectedDesign]);
  useEffect(() => { void load(); }, [load]);
  const request = async (constructorId: string) => {
    if (!selectedDesign) return;
    try { await customerConstructionService.request(selectedDesign, constructorId); setMessage('Construction request sent.'); await load(); }
