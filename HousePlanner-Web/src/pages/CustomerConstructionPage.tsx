...
export default function CustomerConstructionPage() {
 ...
 return <main className="mx-auto max-w-7xl p-4 sm:p-8 space-y-8">
  <header className="flex items-center justify-between"><div><h1 className="text-3xl font-bold">Construction</h1><p className="text-text-muted">Choose a constructor and follow approved projects.</p></div><button aria-label="Refresh construction" onClick={() => void load()} className="rounded-xl border p-3"><RefreshCw size={18}/></button></header>
  {message && <div role="status" className="rounded-xl bg-indigo-50 p-3 text-indigo-800">{message}</div>}
  <section><h2 className="text-xl font-bold mb-3">Approved Designs</h2>{!designs.length ? <p className="rounded-xl border border-dashed p-6 text-text-muted">No designs have been approved yet. Once an architect approves a design, it will appear here for you to request construction.</p> : <div className="grid md:grid-cols-2 gap-3">{designs.map(d => <button key={d.designId} onClick={() => setSelectedDesign(d.designId)} className={`text-left rounded-2xl border p-4 ${selectedDesign===d.designId?'border-emerald-500 ring-1 ring-emerald-500':'bg-surface bg-background'}`}><b>✓ {d.title}</b><p className="text-sm text-text-muted mt-1">{d.bedrooms} Bedrooms · {d.bathrooms} Bathrooms · {d.floorCount} Floors</p><span className="text-xs text-emerald-700">Architect Approved</span></button>)}</div>}</section>
  {design && <section className="space-y-6"><div><h2 className="text-xl font-bold">Review Approved Design</h2><p className="text-sm text-text-muted">Confirm the architect-approved design and estimate before choosing a constructor.</p></div><CostBreakdownCard cost={design.cost}/><div><h2 className="text-xl font-bold">Choose a Constructor</h2><p className="text-sm text-text-muted mb-4">Building: {design.title}</p>{!constructors.length ? <p>No registered constructors are currently available.</p> : <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">{constructors.map(c => <article key={c.id} className="rounded-2xl border bg-surface bg-background p-5"><Building2 className="text-indigo-600"/><h3 className="font-bold mt-3">{c.name}</h3><button onClick={() => void request(c.id)} className="mt-4 w-full rounded-xl bg-indigo-600 py-2.5 text-text-primary font-semibold">Approve &amp; Request Construction</button></article>)}</div>}</div></section>}
  <section><h2 className="text-xl font-bold mb-3">Pending Requests</h2>{!data?.pendingRequests.length?<p className="text-text-muted">No pending requests.</p>:data.pendingRequests.map(r=><div key={r.id} className="rounded-xl border p-4 mb-2"><Clock className="inline mr-2" size={16}/>Pending with <strong>{r.constructorName}</strong><p className="text-sm text-text-muted mt-1 ml-6">Design v{r.designVersion} · Requested {new Date(r.requestedAt).toLocaleDateString()}</p></div>)}</section>
  ...
  <section><h2 className="text-xl font-bold mb-3">Active Construction</h2>{!data?.activeProjects.length?<p className="text-text-muted">No active construction projects.</p>:<div className="grid md:grid-cols-2 gap-3">{data.activeProjects.map(p=><article key={p.id} className="rounded-2xl border p-5"><b>Design v{p.designVersion}</b><p>{p.constructorName}</p><p className="text-sm text-text-muted">Current phase: {p.currentPhase||'Awaiting update'}</p><div className="mt-4 flex gap-3"><Link to={`/dashboard/construction/${p.id}`} className="text-indigo-600 font-semibold">View Progress</Link><button onClick={() => void cancelProject(p.id, p.status === 'in_progress')} className="text-red-600 font-semibold ml-auto hover:underline">Cancel Construction</button></div></article>)}</div>}</section>
