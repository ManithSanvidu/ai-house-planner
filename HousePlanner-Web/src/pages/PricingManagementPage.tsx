...
const GROUP_COLORS: Record<string, string> = {
 ...
 General: 'bg-surface-elevated text-text-secondary ring-1 ring-slate-200',
};

function getGroupColor(group: string): string {
 return GROUP_COLORS[group] ?? 'bg-surface-elevated text-text-secondary ring-1 ring-slate-200';
}
...
const SummaryCard: React.FC<SummaryCardProps> = ({ icon, label, value, sub }) => (
 <div className="bg-surface rounded-2xl border border-border-strong custom-shadow-sm px-6 py-5 flex items-center gap-4">
  ...
  <div>
   <p className="text-[11px] font-semibold uppercase tracking-widest text-text-secondary mb-0.5">{label}</p>
...
const MultiplierBadge: React.FC<MultiplierBadgeProps> = ({ value }) => {
 ...
 const cls = isBase
  ? 'text-text-secondary bg-surface-muted ring-1 ring-border-strong'
  : isHigh
  ? 'text-rose-700 dark:text-rose-400 bg-rose-100 dark:bg-rose-900/30 ring-1 ring-rose-200 dark:ring-rose-800'
  : 'text-purple-700 dark:text-purple-400 bg-purple-100 dark:bg-purple-900/30 ring-1 ring-purple-200 dark:ring-purple-700/50';
...
const CreateModal: React.FC<CreateModalProps> = ({ onClose, onCreateSuccess }) => {
 ...
 return (
  ...
  >
   <div
    className="relative bg-surface rounded-3xl border border-border-strong custom-shadow-lg w-full max-w-lg mx-4 overflow-hidden max-h-[90vh] flex flex-col"
   ...
   >
    ...
    <div className="px-8 pt-8 pb-5 border-b border-border-strong flex-shrink-0">
     <div className="flex items-start justify-between gap-4">
      <div>
       ...
       <h2 className="text-xl font-bold text-text-primary leading-snug">Add Pricing Item</h2>
      ...
      <button
       onClick={onClose}
       className="flex-shrink-0 w-8 h-8 rounded-lg flex items-center justify-center text-text-secondary hover:text-text-secondary hover:bg-surface-elevated transition-colors"
    ...
    <div className="px-8 py-5 space-y-4 overflow-y-auto">
     ...
     {/* Unit Note */}
     <div className="flex items-center gap-2 rounded-xl bg-surface-elevated border border-border/80 px-3.5 py-2.5">
      <Info size={14} className="text-text-secondary flex-shrink-0" />
      <p className="text-xs text-text-secondary">
       {form.category === 'material' ? (
        <>
         <span className="font-semibold text-text-primary">Unit: LKR per sqft</span> (canonical backend value: <code className="text-purple-400 font-mono">per_sqft</code>)
        </>
       ) : (
        <>
         <span className="font-semibold text-text-primary">Unit: Labour factor</span> (canonical backend value: <code className="text-amber-600 font-mono">factor</code>)
        </>
       )}
      </p>
     </div>
     ...
     {/* Item Name */}
     <div>
      <label className="block text-xs font-semibold text-text-secondary mb-1" htmlFor="create-itemName">
       Item Name
      </label>
      <input
       id="create-itemName"
       ...
       disabled={submitting}
       className={`w-full bg-surface-elevated border rounded-xl px-4 py-2 text-sm text-text-primary placeholder-slate-500 outline-none transition-all focus:bg-surface focus:ring-2 focus:ring-purple-500/40 ${
        errors.itemName ? 'border-red-300 ring-1 ring-red-200' : 'border-border focus:border-purple-500'
       }`}
      />
      {errors.itemName && (
     ...
     {/* Display Group */}
     <div>
      <label className="block text-xs font-semibold text-text-secondary mb-1" htmlFor="create-displayGroup">
      ...
      <select
       ...
       disabled={submitting}
       className="w-full bg-surface-elevated border border-border rounded-xl px-3.5 py-2 text-sm text-text-primary outline-none transition-all focus:bg-surface focus:border-purple-500 focus:ring-2 focus:ring-purple-500/40"
      >
     ...
     <div>
      <label className="block text-xs font-semibold text-text-secondary mb-1" htmlFor="create-unitCost">
      ...
      <input
       ...
       disabled={submitting}
       className={`w-full bg-surface-elevated border rounded-xl px-4 py-2 text-sm text-text-primary placeholder-slate-500 outline-none transition-all focus:bg-surface focus:ring-2 focus:ring-purple-500/40 ${
        errors.unitCost ? 'border-red-300 ring-1 ring-red-200' : 'border-border focus:border-purple-500'
       }`}
      />
     ...
     <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
      <div>
       <label className="block text-xs font-semibold text-text-secondary mb-1" htmlFor="create-region">Region</label>
       <input
        id="create-region"
        type="text"
        value={form.region}
        ...
        disabled={submitting}
        className="w-full bg-surface-elevated border border-border rounded-xl px-4 py-2 text-sm text-text-primary outline-none focus:bg-surface focus:border-purple-500"
       />
      </div>
      <div>
       <label className="block text-xs font-semibold text-text-secondary mb-1" htmlFor="create-qualityLevel">Quality Level</label>
       <select
     ...
     {/* Source Reference */}
     <div>
      <label className="block text-xs font-semibold text-text-secondary mb-1" htmlFor="create-sourceReference">
      ...
      <input
       ...
       disabled={submitting}
       className="w-full bg-surface-elevated border border-border rounded-xl px-4 py-2 text-sm text-text-primary placeholder-slate-500 outline-none focus:bg-surface focus:border-purple-500"
      />
     </div>
    </div>
    ...
    <div className="px-8 py-5 border-t border-border-strong flex items-center justify-end gap-3 flex-shrink-0 bg-surface-elevated/50">
     <button
      onClick={onClose}
      disabled={submitting}
      className="px-5 py-2.5 rounded-xl text-sm font-medium text-text-secondary border border-border bg-surface hover:bg-surface-elevated transition-colors disabled:opacity-50"
     >
      Cancel
     </button>
     <button
      ...
      onClick={handleSave}
      disabled={submitting}
      className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-semibold bg-purple-600 hover:bg-purple-500 active:scale-[0.98] text-text-primary transition-all custom-shadow-sm disabled:opacity-60"
     >
...
const EditModal: React.FC<EditModalProps> = ({ item, onClose, onSaveSuccess }) => {
 ...
 return (
  ...
  >
   <div
    className="relative bg-surface rounded-3xl border border-border-strong custom-shadow-lg w-full max-w-lg mx-4 overflow-hidden"
    onClick={(e) => e.stopPropagation()}
   >
    {/* Modal Header */}
    <div className="px-8 pt-8 pb-6 border-b border-border-strong">
     <div className="flex items-start justify-between gap-4">
      <div>
       ...
       <h2 className="text-xl font-bold text-text-primary leading-snug">{item.name}</h2>
    ...
    <div className="px-8 py-6 space-y-4">
     <div>
      ...
      <input
       ...
       onChange={handleChange('unitCost')}
       disabled={saving}
       className={`w-full bg-surface-elevated border rounded-xl px-4 py-2.5 text-sm text-text-primary outline-none transition-all focus:bg-surface focus:ring-2 focus:ring-purple-500/40 ${
        errors.unitCost ? 'border-red-300 ring-1 ring-red-200' : 'border-border focus:border-purple-500'
       }`}
      />
      <p className="mt-1 text-[11px] text-text-secondary">
  ...
  return (
   <div className="space-y-8">
    ...
    <div className="bg-surface rounded-2xl border border-red-100 custom-shadow-sm flex flex-col items-center justify-center py-24 gap-4">
     ...
     <p className="text-sm font-semibold text-text-secondary">Failed to load pricing data</p>
     <p className="text-xs text-text-secondary max-w-sm text-center">{fetchError}</p>
     <button
      id="pricing-retry-btn"
      onClick={fetchPricing}
      className="inline-flex items-center gap-2 mt-2 px-4 py-2 rounded-xl text-xs font-semibold bg-purple-600 text-text-primary hover:bg-purple-500 transition-colors"
     >
      <RefreshCw size={13} />
      Retry
     </button>
   ...
   <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
    ...
    <div className="flex items-center gap-3">
     <button
      ...
      className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-purple-600 hover:bg-purple-500 text-text-primary text-xs font-semibold shadow-sm transition-all"
     >
      <Plus size={15} />
    ...
    <div className="px-6 py-4 border-b border-slate-50 flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
     ...
     <div className="relative w-full sm:max-w-xs">
      ...
      <input
       ...
       className="w-full pl-9 pr-4 py-2.5 text-sm bg-surface-elevated border border-border rounded-xl text-text-primary placeholder-slate-500 outline-none focus:bg-surface focus:border-purple-500 focus:ring-2 focus:ring-indigo-400/30 transition-all"
      />
      {search && (
       <button
        onClick={() => setSearch('')}
      ...
      <thead>
       <tr className="border-b border-border-strong">
        {TABLE_COLUMNS.map((col, i) => (
         <th
          key={i}
          className={`px-5 py-3.5 text-left text-[11px] font-bold tracking-wider text-text-secondary uppercase ${
           i === 0 ? 'pl-6' : ''
      ...
      <tbody className="divide-y divide-slate-800/50">
       {items.length === 0 ? (
        ...
        <tr>
         <td colSpan={TABLE_COLUMNS.length} className="px-6 py-16 text-center">
          <div className="flex flex-col items-center gap-3 text-text-secondary">
           ...
           <p className="text-sm font-medium text-text-secondary">No pricing items are configured yet.</p>
           <p className="text-xs text-text-secondary max-w-sm">
            Add material rates per sq ft and a labour factor to enable the AI Cost Estimation Agent.
           </p>
           <button
            onClick={() => setIsCreateOpen(true)}
            className="inline-flex items-center gap-1.5 mt-2 px-4 py-2 rounded-xl text-xs font-semibold bg-purple-600 text-text-primary hover:bg-purple-500 shadow-sm transition-colors"
           >
            <Plus size={14} />
            Add Pricing Item
           </button>
        ...
        filteredItems.map((item) => (
         <tr
          key={item.id}
          className={`group transition-colors hover:bg-surface-elevated/70 ${
         ...
         >
          ...
          <td className="pl-6 pr-4 py-4">
           <div className="flex items-center gap-2.5">
            <div className="w-7 h-7 rounded-lg bg-surface-muted border border-border flex items-center justify-center flex-shrink-0">
             <Tag size={13} className="text-text-secondary" />
            </div>
            <div>
             <span className="text-sm font-semibold text-text-primary group-hover:text-slate-950 transition-colors">
              {item.name}
             </span>
             {item.sourceReference && (
              <p className="text-[11px] text-text-secondary font-light truncate max-w-xs">{item.sourceReference}</p>
             )}
            </div>
            {savedItemId === item.id && (
          ...
          <td className="px-4 py-4">
           <p className="text-xs font-semibold text-text-secondary">{item.region}</p>
           <p className="text-[11px] text-text-secondary">{item.qualityLevel}</p>
          </td>
          ...
          <td className="px-4 py-4">
           <span className={`inline-flex rounded-full px-2.5 py-0.5 text-[11px] font-semibold ring-1 ${item.isActive ? 'bg-emerald-900/30 text-emerald-300 ring-emerald-700' : 'bg-surface-muted text-text-secondary ring-slate-200'}`}>
          ...
          {/* Canonical Unit */}
          <td className="px-4 py-4">
           <span className="text-xs text-text-secondary font-mono font-medium bg-surface-muted px-2 py-0.5 rounded-md">
            {item.unit}
           </span>
          </td>
          ...
          {/* Rate / Factor */}
          <td className="px-4 py-4">
           <span className={`text-sm font-bold ${item.category === 'labour' ? 'text-amber-300 font-mono' : 'text-text-primary'}`}>
          ...
          <td className="px-4 py-4">
           ...
           <p className="max-w-[140px] truncate text-[10px] text-text-secondary" title={item.updatedByName || (item.updatedByUserId ? 'Unknown user' : 'System')}>
          ...
          <td className="px-4 pr-6 py-4 text-right">
           <div className="flex justify-end gap-1.5">
            <button onClick={() => void handleShowHistory(item)} className="inline-flex items-center gap-1 rounded-lg border border-border px-2 py-1.5 text-xs font-semibold text-text-secondary hover:text-purple-400" aria-label={`History for ${item.name}`}>
             <History size={11} /> History
            </button>
            {item.isActive && <>
             <button onClick={() => setEditingItem(item)} className="inline-flex items-center gap-1 rounded-lg border border-border px-2 py-1.5 text-xs font-semibold text-text-secondary hover:text-purple-400" aria-label={`Edit ${item.name}`}>
              <Pencil size={11} /> Edit
             </button>
     ...
     <div className="w-full max-w-xl rounded-2xl border border-border bg-surface p-6 shadow-xl" onClick={(event) => event.stopPropagation()}>
      <div className="mb-5 flex items-start justify-between gap-4">
       <div>
        ...
        <h2 className="mt-1 text-xl font-bold text-text-primary">{historyItem.name}</h2>
       </div>
        ...
        {history.map((entry) => (
         <div key={entry.id} className="rounded-xl border border-border p-4">
          <div className="flex items-center justify-between gap-3">
           <p className="font-semibold text-text-primary">{entry.previousValue.toLocaleString()} → {entry.newValue.toLocaleString()}</p>
