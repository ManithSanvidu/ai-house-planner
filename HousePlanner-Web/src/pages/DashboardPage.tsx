   ...
   <div className="max-w-[1400px] mx-auto space-y-8">
    ...
    >
     ...
     <div className="flex flex-wrap gap-3">
      ...
      <Link to="/dashboard/plans" className="flex items-center gap-2 px-5 py-3 border border-border rounded-xl font-semibold text-xs tracking-wider text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
       <Library size={16} />
       BROWSE PLANS
      </Link>
      <Link to="/dashboard/new-project" className="flex items-center gap-2 px-5 py-3 bg-blue-600 hover:bg-blue-700 text-text-primary rounded-xl font-semibold text-xs tracking-wider transition-colors shadow-sm shadow-blue-900/20">
       <Plus size={16} />
       START NEW PROJECT
      </Link>
     </div>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
     ...
     <div className="lg:col-span-2 space-y-8">
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }}>
       ...
       {activeProject ? (
        <div className="bg-surface border border-border dark:border-border-strong rounded-3xl p-8 shadow-sm transition-colors">
         ...
         <div className="grid sm:grid-cols-2 gap-4 border-t border-gray-100 dark:border-border-strong pt-6 transition-colors">
          <div>
           <p className="text-xs text-text-secondary uppercase tracking-wider font-bold mb-1 transition-colors">Constructor</p>
           <p className="font-medium text-gray-900 dark:text-text-primary transition-colors">{activeProject.constructorName}</p>
          </div>
          <div>
           <p className="text-xs text-text-secondary uppercase tracking-wider font-bold mb-1 transition-colors">Current Phase</p>
           <p className="font-medium text-gray-900 dark:text-text-primary transition-colors">{activeProject.currentPhase || 'Site Preparation'}</p>
          </div>
         </div>
        </div>
       ) : (
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }}>
       <div className="flex items-center justify-between mb-4">
        <h2 className="text-lg font-bold text-gray-900 dark:text-text-primary transition-colors">Approved Designs</h2>
           ...
           <div key={d.designId} className="bg-surface border border-border dark:border-border-strong rounded-2xl p-5 shadow-sm hover:border-blue-200 dark:hover:border-blue-900/50 transition-colors flex flex-col h-full">
            <div className="flex items-start justify-between mb-4">
             <div className="flex items-center gap-3">
              ...
              <div>
               <h4 className="font-bold text-gray-900 dark:text-text-primary truncate max-w-[150px] transition-colors">{d.title}</h4>
            ...
            <div className="mt-auto flex gap-2">
             <Link to={`/dashboard/workflows/${d.workflowId}?design=${d.designId}`} className="flex-1 py-2 text-center border border-border hover:bg-gray-50 dark:hover:bg-gray-800 text-gray-900 dark:text-text-primary rounded-lg text-xs font-bold transition-colors">
              View
             </Link>
             {isConstructing ? (
              <Link to={`/dashboard/construction`} className="flex-[2] py-2 text-center bg-gray-100 bg-surface-elevated hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-900 dark:text-text-primary rounded-lg text-xs font-bold transition-colors">
               In Construction
              </Link>
             ) : (
     ...
     <div className="space-y-6">
      <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.6 }} className="bg-gradient-to-br from-indigo-50 to-purple-50 dark:from-indigo-950/30 dark:to-purple-950/30 border border-indigo-100 dark:border-indigo-900/50 rounded-3xl p-6 shadow-sm transition-colors">
       ...
       <h3 className="text-lg font-bold text-gray-900 dark:text-text-primary mb-2 transition-colors">Next Step</h3>
      ...
      <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.7 }} className="bg-surface border border-border dark:border-border-strong rounded-3xl p-6 shadow-sm transition-colors">
       ...
       <ul className="space-y-3">
        ...
        <li><Link to="/dashboard/plans" className="text-sm text-text-secondary hover:text-blue-600 dark:hover:text-blue-400 flex items-center justify-between group transition-colors">Plan Catalogue <ArrowRight size={14} className="opacity-0 group-hover:opacity-100 transition-opacity" /></Link></li>
        <li><Link to="/settings" className="text-sm text-text-secondary hover:text-blue-600 dark:hover:text-blue-400 flex items-center justify-between group transition-colors">Account Settings <ArrowRight size={14} className="opacity-0 group-hover:opacity-100 transition-opacity" /></Link></li>
       </ul>
      </motion.div>
     </div>
