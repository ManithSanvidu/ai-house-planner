...
export const ConstructorRequestsView = ({ projectId }: { projectId: string }) => {
  ...
  if (loading) return <div className="text-sm text-gray-500 py-2">Loading constructor requests...</div>;
  if (error) return <div className="text-sm text-red-500 py-2">{error}</div>;
  
  if (requests.length === 0) {
    return (
      <div className="mt-4 rounded-xl border border-dashed border-gray-300 bg-gray-50 p-6 text-center dark:border-gray-700 dark:bg-gray-800/50">
        <p className="text-sm text-gray-500 dark:text-gray-400">No constructor requests yet.</p>
        <p className="text-xs text-gray-400 mt-1">Share Project ID <strong>{projectId}</strong> with your constructor.</p>
      </div>
    );
  }
  ...
  return (
    <div className="mt-4 rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900">
      <div className="border-b border-gray-100 bg-gray-50 px-4 py-3 dark:border-gray-800 dark:bg-gray-800/50 flex justify-between items-center">
        <h3 className="text-sm font-semibold text-gray-900 dark:text-white">Constructor Requests</h3>
          ...
          <div key={req.id} className="flex items-center justify-between p-4">
            ...
            {req.status === 'pending' && (
              <button
                ...
                className="flex items-center gap-1 rounded-lg bg-indigo-600 px-3 py-1.5 text-xs font-semibold text-white shadow-sm hover:bg-indigo-500"
              >
