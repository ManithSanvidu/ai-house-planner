...
export const ConstructorProjectCalendar: React.FC<ConstructorProjectCalendarProps> = ({ 
 ...
 const renderCells = () => {
  ...
  for (let d = 1; d <= daysInMonth; d++) {
   ...
   cells.push(
    <div 
     key={d} 
     className={`min-h-[100px] border border-border bg-surface-elevated p-1 flex flex-col ${!isReadOnly ? 'cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700' : ''}`}
     onClick={(e) => {
    ...
    >
     <div className="flex justify-between items-center mb-1 px-1">
      <span className={`text-sm font-medium ${isToday(d) ? 'bg-indigo-600 text-text-primary rounded-full w-6 h-6 flex items-center justify-center' : 'text-gray-700 dark:text-gray-300'}`}>
       {d}
      </span>
      {!isReadOnly && (
        <span 
         className="text-xs text-text-secondary hover:text-indigo-600 cursor-pointer hidden md:block opacity-0 group-hover:opacity-100 transition-opacity"
 ...
 return (
  <div className="bg-surface-elevated rounded-xl p-4 md:p-6 shadow-sm ring-1 ring-gray-900/5 dark:ring-white/10">
   
   <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
    <div className="flex items-center gap-4">
      <button onClick={prevMonth} className="p-1 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700 text-text-secondary dark:text-gray-300">
       <ChevronLeft className="w-5 h-5" />
      </button>
      <h2 className="text-xl font-bold text-gray-900 dark:text-text-primary w-40 text-center">
       {monthNames[month]} {year}
      </h2>
      <button onClick={nextMonth} className="p-1 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700 text-text-secondary dark:text-gray-300">
       <ChevronRight className="w-5 h-5" />
      </button>
      
      <button onClick={goToToday} className="px-3 py-1 text-sm border border-border-strong rounded-md hover:bg-gray-50 dark:border-gray-600 dark:hover:bg-gray-700 dark:text-gray-200 ml-2">
       Today
      </button>
    </div>
    
    <div className="flex flex-wrap gap-3 text-xs font-medium text-text-secondary dark:text-gray-300">
