...
export const ConstructorProjectDetails: React.FC = () => {
  ...
  if (!project) {
    return <div className="p-8 text-center text-slate-500">Project not found or unauthorized.</div>;
  }
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-8 p-4 md:p-8 min-h-[calc(100vh-64px)] bg-[#0B0F19] text-slate-200">
      <Link
      ...
      {/* Project Overview */}
      <div className="overflow-hidden rounded-2xl bg-[#111827] border border-slate-800 shadow-lg p-6">
        <h2 className="text-xl font-bold text-white mb-6">Project Overview</h2>
        <div className="grid grid-cols-2 gap-6 sm:grid-cols-4">
          <div className="bg-[#1F2937]/50 rounded-xl p-4 border border-slate-700/50">
            <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1">Status</p>
            <p className="font-bold text-white capitalize">{project.status}</p>
          </div>
          <div className="bg-[#1F2937]/50 rounded-xl p-4 border border-slate-700/50">
            <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1">Current Phase</p>
            <p className="font-bold text-white">{currentPhaseName}</p>
          </div>
          <div className="bg-[#1F2937]/50 rounded-xl p-4 border border-slate-700/50">
            <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1">Overall Progress</p>
          ...
          <div className="bg-[#1F2937]/50 rounded-xl p-4 border border-slate-700/50">
            <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1">Planned Completion</p>
      ...
      {/* Construction Phases */}
      <div className="overflow-hidden rounded-2xl bg-[#111827] border border-slate-800 shadow-lg">
        <div className="px-6 py-5 border-b border-slate-800 flex justify-between items-center bg-[#1F2937]/30">
          <h2 className="text-lg font-bold text-white">Construction Phases Schedule</h2>
        ...
        <div className="divide-y divide-slate-800/50">
          {project.constructionPhases.map((phase: ConstructionPhase) => (
            <div key={phase.id} className="flex flex-col sm:flex-row sm:items-center sm:justify-between px-6 py-5 gap-4 hover:bg-[#1F2937]/30 transition-colors">
                ...
                {editingPhaseId === phase.id ? (
                  <div className="flex items-center gap-2">
                    <input
                      type="number"
                      min="1"
                      className="w-16 rounded-lg bg-[#0B0F19] border border-slate-700 focus:border-purple-500 focus:ring-purple-500 text-sm text-white px-2 py-1"
                    ...
                    <button
                      ...
                      className="text-xs bg-purple-600 text-white px-2.5 py-1.5 rounded-lg hover:bg-purple-500 disabled:opacity-50 font-semibold"
                      disabled={savingPhase}
                    >
                      Save
                    </button>
                    <button
                      ...
                      className="text-xs bg-slate-700 text-slate-300 px-2.5 py-1.5 rounded-lg hover:bg-slate-600 font-semibold"
                      disabled={savingPhase}
                    >
                      Cancel
                    </button>
                  ...
                  <div className="flex flex-col items-end">
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-medium text-slate-300">Planned: {phase.plannedDurationDays}d</span>
                      {!isReadOnly && (
                        <button
                          onClick={() => {
                          ...
                          className="text-slate-500 hover:text-purple-400 text-xs font-semibold uppercase tracking-wider"
                        >
                          Edit
                        </button>
                      )}
                  ...
                  {isReadOnly || phase.status === 'Completed' ? (
                    ...
                    <select
                      ...
                      className={`text-xs font-bold rounded-lg border-0 ring-1 ring-inset focus:ring-2 focus:ring-inset py-1.5 pl-3 pr-8 ${
                        ...
                        'bg-[#0B0F19] text-slate-300 ring-slate-700 focus:ring-purple-500'
                      }`}
                    >
          ...
          {project.constructionPhases.length === 0 && (
            <div className="px-6 py-8 text-center text-slate-500">No phases defined.</div>
          )}
        </div>
      </div>
      ...
      {/* Daily Logbook */}
      <div className="overflow-hidden rounded-2xl bg-[#111827] border border-slate-800 shadow-lg">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between px-6 py-5 border-b border-slate-800 gap-4 bg-[#1F2937]/30">
          <div>
            <h2 className="text-lg font-bold text-white">Daily Site Progress</h2>
            <p className="text-xs text-slate-400 mt-1">Record daily work, workforce, materials, challenges, and progress.</p>
          </div>

          <div className="flex items-center gap-4 w-full sm:w-auto">
            {!showForm && (
              <div className="flex rounded-lg shadow-sm ring-1 ring-slate-700">
                <button
                ...
                <button
                  ...
                  className={`px-4 py-1.5 text-xs font-bold uppercase tracking-wider rounded-r-lg transition-colors border-l border-slate-700 ${
                    activeTab === 'calendar'
                      ? 'bg-[#374151] text-white'
                      : 'bg-[#1F2937] text-slate-400 hover:text-white'
                  }`}
            ...
            {!isReadOnly && !showForm && (
              <button
                ...
                className="inline-flex items-center rounded-lg bg-purple-600 px-3 py-1.5 text-xs font-bold text-white shadow-sm hover:bg-purple-500 transition-colors uppercase tracking-wider"
              >
        ...
        <div className="p-6">
          {showForm ? (
            ...
            <div className="space-y-4">
              {logs.length === 0 ? (
                <div className="text-center py-12 text-slate-500 bg-[#1F2937]/30 rounded-xl border border-slate-800 border-dashed">
                ...
                logs.map(log => {
                  ...
                  return (
                  <div key={log.id} className="rounded-xl border border-slate-800 p-5 bg-[#1F2937]/40 hover:bg-[#1F2937]/60 transition-colors">
                    <div className="flex justify-between items-start mb-4 pb-4 border-b border-slate-800/50">
                      <div className="flex items-center text-sm font-bold text-white">
                        ...
                        {log.phaseName && (
                          <span className={`ml-4 px-2.5 py-1 rounded-md text-[10px] uppercase tracking-wider font-bold ${isPhaseInProgress ? 'bg-purple-900/30 text-purple-400 ring-1 ring-purple-800' : 'bg-slate-800 text-slate-400 ring-1 ring-slate-700'}`}>
                            {log.phaseName}
                          </span>
                        )}
                      </div>
                      {!isReadOnly && (
                        <div className="flex gap-1">
                          <button onClick={() => { setEditingLog(log); setShowForm(true); }} className="text-slate-500 hover:text-purple-400 p-1.5 rounded-lg hover:bg-slate-800 transition-colors">
                            <Edit2 className="h-3.5 w-3.5" />
                          </button>
                          <button onClick={() => handleDelete(log.id)} className="text-slate-500 hover:text-rose-400 p-1.5 rounded-lg hover:bg-slate-800 transition-colors">
                    ...
                    <div className="space-y-4 text-sm">
                      <div>
                        <span className="font-semibold text-slate-300 block mb-1">Work Completed</span>
                        <p className="text-slate-400">{log.workCompleted}</p>
                      </div>
                      
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {log.challenges && (
                          <div>
                            <span className="font-semibold text-slate-300 block mb-1 flex items-center gap-1.5">
                              <span className="w-1.5 h-1.5 rounded-full bg-rose-500"></span> Challenges
                            </span>
                            <p className="text-slate-400">{log.challenges}</p>
                          </div>
                        )}
                        {log.materialsUsed && (
                          <div>
                            ...
                            <p className="text-slate-400">{log.materialsUsed}</p>
                          </div>
                        )}
                      </div>

                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-4 p-4 bg-[#0B0F19] rounded-xl border border-slate-800/80">
                         <div>
                           <p className="text-[10px] uppercase font-bold tracking-widest text-slate-500 mb-1">Progress</p>
                           <p className="font-bold text-white text-base">{log.progressPercentage}%</p>
                         </div>
                         <div>
                           <p className="text-[10px] uppercase font-bold tracking-widest text-slate-500 mb-1">Workers</p>
                           <p className="font-bold text-white text-base">{log.workforceCount || '-'}</p>
                         </div>
                         <div className="col-span-2">
                           {log.tomorrowPlan && (
                             <>
                               <p className="text-[10px] uppercase font-bold tracking-widest text-slate-500 mb-1">Tomorrow's Plan</p>
                               <p className="text-slate-300 text-xs mt-0.5">{log.tomorrowPlan}</p>
                             </>
                           )}
                         </div>
                      </div>
