...
export default function CustomerConstructionProgressPage() {
 ...
 <section className="rounded-2xl border p-5 overflow-x-auto"><h2 className="text-xl font-bold">Construction Activity</h2><p className="text-sm text-zinc-500 mb-4">Color intensity shows the number of recorded updates, not construction quality.</p><div className="grid grid-rows-7 grid-flow-col gap-1 w-max">{days.map(d=>{const key=d.getFullYear()+'-'+String(d.getMonth()+1).padStart(2,'0')+'-'+String(d.getDate()).padStart(2,'0');const a=activity.get(key);const n=a?.count||0;const color=n===0?'bg-zinc-100 dark:bg-gray-800':n===1?'bg-emerald-300':n===2?'bg-emerald-500':'bg-emerald-700';return <div key={key} title={`${key}: ${n} construction update${n===1?'':'s'}`} aria-label={`${key}: ${n} construction updates`} className={`h-3.5 w-3.5 rounded-sm ${color}`}/>})}</div></section>
  <section><h2 className="text-xl font-bold mb-4">Construction Updates</h2>{!data.logs.length?<p className="text-zinc-500">No construction updates yet.</p>:<div className="space-y-3">{data.logs.map((l:any)=><article key={l.id} className="rounded-2xl border p-5"><time className="text-sm text-zinc-500">{new Date(l.date).toLocaleDateString()}</time><h3 className="font-bold">{l.phase||'Project update'}</h3><p className="mt-2">{l.completedWork}</p><p className="text-sm text-indigo-600 mt-2">Progress recorded: {l.progressPercentage}%</p>{l.challenges&&<p className="text-sm mt-2 font-semibold">Challenges: <span className="font-normal">{l.challenges}</span></p>}{l.issues&&<p className="text-sm text-amber-700 mt-1">Issue: {l.issues}</p>}{l.resolution&&<p className="text-sm text-emerald-700 mt-1">Resolution: {l.resolution}</p>}{l.tomorrowPlan&&<p className="text-sm mt-1">Tomorrow's Plan: {l.tomorrowPlan}</p>}{l.additionalNotes&&<p className="text-sm text-zinc-500 mt-1">Notes: {l.additionalNotes}</p>}</article>)}</div>}</section></main>;
}
